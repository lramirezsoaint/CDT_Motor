...
export function classifyIncident(input: IncidentClassificationInput): IncidentClassification {
  ...
  if (/session expired|sesion expirada|no hay distribuciones disponibles/i.test(text)) {
    return result('AUTH_FAILURE', 'SESSION_EXPIRED', 'HIGH', 'La sesion o estado base del entorno no esta disponible.');
  }

  if (/enoent|no such file|fixture .*no existe|fixture .*not found|missing fixture|archivo no existe|dato previo faltante|registro previo no existe|no existe una distribucion|missing test data/i.test(text)) {
