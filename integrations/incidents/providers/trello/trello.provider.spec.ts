...
test.describe('trello provider', () => {
  test('mapper builds card with signature', () => {
    ...
    expect(card.name).toContain('[QA-AUTO][E37-AM-01.01.1]');
    expect(card.desc).toContain('Bloque: Bloque 5');
  ...
  test('description without signature is not duplicate', () => {
    const duplicate = findTrelloDuplicateBySignature(makeProviderTestCandidate(), [{ id: 'card-1', name: 'no signature', desc: 'Caso: E37-AM-01.01.1' }]);
