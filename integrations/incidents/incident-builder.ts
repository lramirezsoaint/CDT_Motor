...
const CASE_ID_PATTERN = /\b(E\d+(?:-[A-Z0-9]+)*(?:\.\d+)*)\b/i;
...
function inferBlock(specFile: string): string {
  const match = normalizePath(specFile).match(/tests\/e2e\/bloque-([^/]+)/i);

  if (!match) {
    return 'N/D';
  }

  return /^\d+$/.test(match[1]) ? `Bloque ${match[1]}` : `Bloque ${match[1]}`;
}
