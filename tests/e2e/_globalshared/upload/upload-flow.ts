...
export function defineSharedUploadCase(config: SharedUploadCaseConfig): void {
  test.describe(`@critical @${config.bloqueTag} @${config.moduleTag} @upload @${config.caseId}`, () => {
    test(`@critical @${config.bloqueTag} @${config.moduleTag} @upload @${config.caseId} debe cargar archivo en ${config.view}`, async ({
      page,
    }) => {
