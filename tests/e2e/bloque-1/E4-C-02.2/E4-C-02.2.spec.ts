...
import scenarioData from '@data/bloque-1/E4-C-02.2.json';
import {
  acceptButtonFrom,
  goToComunesView,
  openUploadDialog,
...
test.describe('@comunes @critical @E4-C-02.2', () => {
  test('@critical @comunes @E4-C-02.2 debe cargar el archivo de Grupo Producto cuando la aplicacion no tiene datos previos', async ({
    page,
  }) => {
    ...
    await test.step('QA: verificar que la tabla de Grupo Producto no contiene datos previos', async () => {
      const table = page.locator('table').first();
      await expect(table, '[BUG] La tabla de Grupo Producto no esta visible al ingresar a la vista.').toBeVisible({ timeout: 15_000 });
      ...
      if (rowCount > 0) {
        test.info().annotations.push({
          type: 'observacion',
          description: `La tabla de Grupo Producto ya contiene ${rowCount} fila(s) antes de la carga. El test continuara validando que la carga incrementa los datos correctamente.`,
        });
      }
    });

    await test.step('QA: abrir "Cargar Grupo Producto" y validar estado inicial del modal', async () => {
    ...
    await test.step('QA: seleccionar archivo valido y validar estados intermedios del modal', async () => {
      ...
      const uploadedFile = await uploadFile(page, {
        fixtureDir: 'fixtures/files/bloque-1/E4-C-02.2',
    ...
    await test.step('QA: confirmar la carga y validar mensaje de exito', async () => {
      ...
      try {
        await validateSuccessfulUpload(page, { entityName: 'Grupo Producto', timeoutMs: 60_000, closeWithAccept: true });
      } catch {
        test.info().annotations.push({
          type: 'ambiente',
          description:
