...
test.describe('@distribucion @critical @E63-C-BUS', () => {
  scenarioData.busquedaGeneral.forEach((searchCase) => {
    test(`@critical @distribucion @E63-C-BUS ${searchCase.id} debe validar búsqueda general en Distribución`, async ({
      page,
    }) => {
  ...
  scenarioData.filtrosDistribucion.forEach((filterCase) => {
    test(`@critical @distribucion @E63-C-BUS ${filterCase.id} debe validar el filtro ${filterCase.etiqueta}`, async ({
      page,
    }) => {
  ...
  scenarioData.autocompletado.forEach((autocompleteCase) => {
    test(`@critical @distribucion @E63-C-BUS ${autocompleteCase.id} debe validar autocompletado en Buscar`, async ({
      page,
    }) => {
      await test.step('Acceder a la página de inicio Distribución', async () => {
        await openDistribution(page);
