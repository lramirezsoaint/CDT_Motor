...
export function DeleteExactusVersionCase(config: ExactusCaseConfig) {
 test(`${tagsFor({ ...config, section: 'Aprovisionamiento', view: 'Exactus', flowTag: '@eliminar' })} borra version Exactus`, async ({ page }) => {
...
export function ExactusProcessedDownloadCase(config: ExactusCaseConfig) {
 test(`${tagsFor({ ...config, section: 'Aprovisionamiento', view: 'Exactus', flowTag: '@download_reporte' })} descarga Exactus procesado`, async ({ page }) => {
 test.setTimeout(120_000);
 await openExactus(page);
...
export function ExactusProcessedFlowCase(config: ExactusCaseConfig) {
 test(`${tagsFor({ ...config, section: 'Aprovisionamiento', view: 'Exactus', flowTag: '@procesos' })} carga Exactus procesado y valida procesos`, async ({ page }, testInfo) => {
