...
const uploadCase = AM_DATA_UPLOAD_CASES['E8-AM-01.2'];
test.use({ storageState: '.auth/gestorAM.json' });
test('@bloque5 @E8-AM-01.2 @am @upload_incompleto @gestor-asiento @critical rechaza datos incompletos en Partidas', async ({ page }) => {
  ...
  await test.step('Cargar Partidas con datos obligatorios incompletos', async () => { await runAmDataUpload(page, uploadCase); });
});
