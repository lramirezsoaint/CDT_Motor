...
const uploadCase = AM_DATA_UPLOAD_CASES['E9-AM-01.2'];
test.use({ storageState: '.auth/gestorAM.json' });
test('@bloque5 @E9-AM-01.2 @am @tabla @gestor-asiento @regression muestra Partidas cargadas', async ({ page }) => {
  ...
  await test.step('Cargar Partidas y comparar la tabla con el archivo', async () => { await runAmDataUpload(page, uploadCase); });
});
