...
export function defineAmDownloadCase(downloadCase: AmDownloadCase): void {
  test.describe(`@bloque5 @${downloadCase.caseId}`, () => {
    test(`${buildTags({ bloque: '@bloque5', caseId: downloadCase.caseId, flowTag: downloadCase.flowTag ?? '@download_catalogo' })} debe validar descarga de ${downloadCase.entityName}`, async ({
