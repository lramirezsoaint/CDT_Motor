...
async function openAmView(page: Page, config: Pick<AddRecordCaseConfig, 'section' | 'view'>) {
  const amPage = new Bloque5AsientosManualesPage(page);
  await amPage.openSidebarView(sectionName(config.section), config.view);
}

function visibleButton(page: Page, name: RegExp) {
...
function tagsFor(config: Pick<AddRecordCaseConfig, 'caseId'>) {
  return `@bloque5 @${config.caseId} @am @agregar @gestor-asiento @regression`;
}
