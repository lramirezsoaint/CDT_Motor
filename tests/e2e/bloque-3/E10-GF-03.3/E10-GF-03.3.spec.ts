...
test.describe('@bloque3 @gf @critical @E10-GF-03.3', () => {
  test('@critical @bloque3 @gf @E10-GF-03.3 debe validar descarga de archivo Distribucion UoA Generales Multiramo', async ({
    ...
    await test.step('Validar que exista boton de descarga', async () => {
      ...
      await expect(downloadButton, 'Debe existir el botón Descargar en Distribucion UoA Generales Multiramo.').toBeVisible({ timeout: 20_000 });
    });

    await test.step('Ejecutar descarga del archivo si el botón está habilitado', async () => {
      const downloadButton = page.locator('[data-testid*="download"], button[id*="download"], [aria-label*="descargar" i]').first();
      await expect(downloadButton, 'Debe existir el botón Descargar en Distribucion UoA Generales Multiramo.').toBeVisible({ timeout: 15_000 });
      const isEnabled = await downloadButton.isEnabled();
      if (!isEnabled) {
        test.info().annotations.push({ type: 'skip', description: 'Botón descargar deshabilitado para Distribucion UoA Generales Multiramo.' });
        return;
      }
