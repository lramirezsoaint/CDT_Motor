...
test.describe('@bloque3 @gf @critical @E9-GF-02.1', () => {
  test('@critical @bloque3 @gf @E9-GF-02.1 debe validar visualizacion de datos de Exactus cargados', async ({
   ...
   try {
    ...
    await test.step('Validar que se muestre tabla con estructura de columnas', async () => {
      ...
      await test.expect(table, 'Debe existir una tabla de Exactus.').toBeVisible({ timeout: 20_000 });
    ...
    await test.step('Validar que la tabla contenga datos cargados', async () => {
      ...
      test.expect(rowCount, 'La tabla debe mostrar registros de Exactus.').toBeGreaterThan(0);
    });
    ...
    await test.step('Validar que los datos mostrados coincidan con estructura esperada', async () => {
      ...
      await test.expect(cells.first(), 'La primera celda debe contener datos de Exactus.').toBeVisible({ timeout: 10_000 });
      
      const firstCellText = await cells.first().innerText();
      test.expect(firstCellText.trim().length, 'El dato de Exactus no debe estar vacío.').toBeGreaterThan(0);
    });
  });
});