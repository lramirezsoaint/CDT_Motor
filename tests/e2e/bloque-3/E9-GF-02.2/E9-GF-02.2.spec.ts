...
test.describe('@bloque3 @gf @critical @E9-GF-02.2', () => {
  test('@critical @bloque3 @gf @E9-GF-02.2 debe validar visualizacion de datos de Driver Producto cargados', async ({
    ...
    await test.step('Validar que se muestre tabla con estructura de columnas', async () => {
      ...
      await test.expect(table, 'Debe existir una tabla de Driver Producto.').toBeVisible({ timeout: 20_000 });
    ...
    await test.step('Validar que la tabla contenga datos cargados', async () => {
      ...
      test.expect(rowCount, 'La tabla debe mostrar registros de Driver Producto.').toBeGreaterThan(0);
    });
    ...
    await test.step('Validar que los datos mostrados coincidan con estructura esperada', async () => {
      ...
      await test.expect(cells.first(), 'La primera celda debe contener datos de Driver Producto.').toBeVisible({ timeout: 10_000 });

      const firstCellText = await cells.first().innerText();
      test.expect(firstCellText.trim().length, 'El dato de Driver Producto no debe estar vacío.').toBeGreaterThan(0);
    });
  });
});